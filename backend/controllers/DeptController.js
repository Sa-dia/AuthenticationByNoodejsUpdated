...
const uploadDeptAsXML = async (req, res) => {
    ...
    xml2js.parseString(xmlData, async (err, result) => {
        ...
        try {
            await clearTable('xml_dept_data'); // Clear the table before inserting new data
            ...
            for (const row of rows) {
                ...
                if (Dept_Name && Descript && Phone && Fax && Email) {
                    await insertXmlDeptIntoDatabase({ Id, Dept_Name, Descript, Phone, Fax, Email });
                } else {
...
const insertXmlDeptIntoDatabase = (row) => {
    return new Promise((resolve, reject) => {
        const query = 'INSERT INTO xml_dept_data (Id, Dept_Name, Descript, Phone, Fax, Email) VALUES (?, ?, ?, ?, ?, ?)';
        db.query(query, [row.Id, row.Dept_Name, row.Descript, row.Phone, row.Fax, row.Email], (err, results) => {
            if (err) {
                reject(err);
            } else {
                resolve(results);
